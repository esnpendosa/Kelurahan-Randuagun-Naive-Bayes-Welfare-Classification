	...
	for i, row := range rows {
		...
		if errEval == nil {
			for _, r := range evalRows {
				if len(r) > 1 && strings.EqualFold(strings.TrimSpace(r[1]), name) {
					if len(r) > 9 {
		...
		if !isUji && errEval2 == nil {
			for _, r := range evalRows2 {
				if len(r) > 1 && strings.EqualFold(strings.TrimSpace(r[1]), name) {
					if len(r) > 9 {
						excelVal2 := strings.TrimSpace(r[9])
						...
						if foundPred2 {
							...
							for cc := 1; cc <= 6; cc++ {
								excelColIdx2 := cc + 1
								if excelColIdx2 < len(r) {
									valStr2 := strings.TrimSpace(r[excelColIdx2])

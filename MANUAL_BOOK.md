...
## 📈 Bagian 11: Lampiran Diagram Sistem (Format PlantUML)

Bagian ini ditujukan untuk kebutuhan dokumentasi skripsi (ERD, Flowchart, dan Use Case) dengan format **PlantUML**.

### 11.1 Entity Relationship Diagram (ERD)
Diagram ini menunjukkan hubungan antar tabel dalam database aplikasi. Terdapat 4 tabel utama: Pengguna, Warga, Data Indikator, dan Hasil Klasifikasi.
...
### 11.2 Flowchart Sistem
Flowchart ini menjelaskan alur penggunaan sistem dari awal login hingga menampilkan hasil klasifikasi Naive Bayes dan Laporan.
...
fork again
  ...
  if (Semua indikator diisi?) then (Ya)
    :Proses Naive Bayes;
    note right
      1. Hitung Probabilitas Prior
      2. Hitung Probabilitas Likelihood
      3. Pilih probabilitas tertinggi (Kelas 1-6)
    end note
    :Tampilkan hasil & Grafik;
    :Simpan ke riwayat;
  else (Belum)
    :Teringatan wajib isi semua;
  endif

fork again
  :=== TRAINING MODEL ===;
  if (Data latih >= 60?) then (Ya)
    :Latih ulang model;
    :Tampilkan Akurasi\n(Confusion Matrix);
  else (Tidak)
...
Diagram ini memperlihatkan interaksi antara Aktor (Admin) dengan sistem.
...
rectangle "Sistem Klasifikasi Kesejahteraan" {
    usecase "Login / Logout" as UC1
    usecase "Kelola Data Warga" as UC2
    usecase "Import Excel Data Warga" as UC3
    usecase "Klasifikasi Kesejahteraan" as UC4
    usecase "Training Model (Naive Bayes)" as UC5
    usecase "Lihat Laporan & Export" as UC6
    usecase "Manajemen Akun Admin" as UC7
}
